...
public class NPCParkingBehaviour : NPCLaneFollowBehaviour
{
    ...
    private State stateBeforeReversing;
    ...
    private float lastTimeMovedBackward;
    ...
    private IEnumerator TryParkCorutine()
    {
        ...
        while ((CurrentState == State.LookingForparking && ParkingNPCManager.NPCParkingBehaviourEnabled))
        {
            TryPark();
            yield return new WaitForSeconds(0.3f);
        }
    ...
    public bool TryInitLeaving(bool force = false)
    {
        if (!force && Time.time - timeWhenParked < ParkingNPCManager.MinimumAmountOfTimeCarMustBeParked)
        {
            return false;
        }
    ...
    protected override void SetTargetTurn()
    {
        ...
        else if (CurrentState == State.IsLeaving)
        {
            if (reverse)
            {
                ...
                if (progress > -0.5f)
                {
                    targetTurn = 15 * projectionOnRight;
                }
                else
                {
                    targetTurn = 70 * projectionOnRight;
                }
                if (Math.Abs(projectionOnRight) < 0.1)
                {
                    GoToLane();
                }
                currentTurn = Mathf.Lerp(currentTurn, targetTurn, 0.5f);
    ...
    protected override void SetTargetSpeed()
    {
        ...
        else if (CurrentState == State.IsReversing)
        {
            ...
            if (Physics.Raycast(front, transform.forward, out var hit, 10, 1 << LayerMask.NameToLayer("NPC")))
            {
                var behaviour = hit.collider.GetComponent<NPCParkingBehaviour>();
                if (behaviour.CurrentState == State.IsLeaving)
                {
                    //OK
                }
                else
                {
                    CurrentState = stateBeforeReversing;
                }
            }
            else
            {
        ...
        else if (CurrentState == State.IsLeaving)
        {
            ...
            {
                currentSpeed = targetSpeed = -2;
                if (Vector3.Dot((transform.position - closest), (middleMiddle - closestFromparking).normalized) < 0f)
                {
                    GoToLane();
                }
                ...
                {
                    lastTimeMovedBackward = Time.time;
                }
                else if (Time.time - lastTimeMovedBackward > 3)
                {
                    if (distanceToLane < 1)
                    {
                        GoToLane();
    ...
    {
        if (CurrentState == State.LaneFollowing || CurrentState == State.IsParking || CurrentState == State.LookingForparking)
        {
            stateBeforeReversing = CurrentState;
            CurrentState = State.IsReversing;
        }
    }
    public void EditParkTimeMessage(float time)
    {
        timeWhenParked = time;
    }
    private void GoToLane()
    {
        ...
        var firstOnLane = ClosestPoint(controller.frontCenter.position, currentMapLane);
        parkingPath.Add(firstOnLane);
        parkingPath.Add(NextPoint(parkingPath.Last(), currentMapLane, 10));
        var path = new CatmullRom(parkingPath, 20, false);
        way = path.GetPoints().Select(s => s.position).ToList();
        SetLaneData(way);
    ...
    public override void Despawn()
    {
        ...
        if (CurrentState == State.IsParked)
        {
            //It can happen from FixedUpdate just after switched to parked
            return;
        }
        ForceDespawn();
    }
